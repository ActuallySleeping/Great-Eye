...
const generate = async (message, server) => {
    ...
    const send = (i) => {
        return p.slice(i, i + 20)
            .map(player => ` ${p.indexOf(player) + 1 < 10 ? '0' + (p.indexOf(player) + 1) : p.indexOf(player) + 1} ` +
...
class scan {
    ...
    async execute(message, args, client) {
        ...
        if (!response)
            message.reply('Error with the query').then(msg => { setTimeout(() => msg.delete().catch(e => { return; }), 4 * 1000); });
                            ...
                            setTimeout(() => {
                                message.channel.messages.fetch({ limit: 1 }).then(async (msg) => {
                                    if (!msg.first().author.bot) {
                                        ...
                                        if (!selected.match(new RegExp(/^[0-9]$/g)))
