...
import { Message, MessageEmbed, Client, PermissionResolvable, CommandInteractionOptionResolver } from "discord.js";
...
const httpGetAsync = (url : string, callback : CallableFunction) => {
...
export class scan implements command {
	...
	execute(message : Message, args : String[], client : Client, localDB : Database) {
        ...
        httpGetAsync(serversQuery, (response) => {
            if (!response) return message.reply("No servers found");
            ...
            {
                message.reply(
                    "** The request need to be more specific. **" + '\n' +
                    "```"+
